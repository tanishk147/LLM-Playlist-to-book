...
Up to this point, we have built each piece of the GPT architecture in isolation: tokenizers, embedding layers, positional encodings, masked multi-head attention, feed-forward networks, layer normalization, and shortcut connections. This chapter is where everything comes together. We will assemble these components into a single, coherent `GPTModel` class, trace a tensor through the entire forward pass, count the model's parameters, and finally implement the autoregressive text generation loop that turns raw logits into readable text.
...
[FIGURE: End-to-end GPT pipeline: input token IDs → token embedding lookup → positional embedding lookup → elementwise sum → dropout → 12× transformer block → layer norm → linear output head → logits tensor of shape (batch, seq_len, vocab_size)]
...
Rather than hard-coding hyperparameters throughout the model, we collect them in a single dictionary. For the 124-million-parameter GPT-2 variant, the configuration is:
...
- **`vocab_size: 50257`** — The number of unique tokens the model knows about. Each token ID is an integer in the range `[0, 50256]` [c438].
...
This single dictionary is passed to every sub-module, making it easy to experiment with different model sizes by swapping one configuration for another.

---

### A Dummy Model First: Validating the Architecture

Before implementing the full model with all its internals, it is instructive to build a **dummy version** that has the correct overall structure but uses placeholder sub-modules. This lets us verify that tensor shapes flow correctly through the pipeline before we worry about the details of each component.
...
The `DummyTransformerBlock` and `DummyLayerNorm` are stand-ins; in the real model they are replaced by the fully implemented versions we built in earlier chapters. The transformer block is the most important part of the large language model architecture and consists of multiple different aspects linked together [c366], and the transformer block is the key component of the entire GPT model [c920].
...
The first two layers of the model are embedding tables.

**Token embedding:** An `nn.Embedding` layer of shape `(vocab_size, emb_dim)` maps each integer token ID to a learned dense vector. In GPT-2, the embedding layer is trained to learn token representations that capture semantic meaning [c388].

**Positional embedding:** A second `nn.Embedding` layer encodes position information. Positional embedding is a vector representation of the position of a token within a sequence [c926]. The positional embedding table is created as:
...
Token embeddings and positional embeddings are summed to produce input embeddings for each token [c820]. Intuitively, this additive combination means the model receives both *what* a token is (semantic content) and *where* it appears (position) in a single vector.
...
The heart of the model is a sequence of transformer blocks. The preprocessing pipeline before the transformer block consists of: tokenization, token embedding, positional embedding, input embedding computation, and dropout [c939]. After that preprocessing, the tensor enters the transformer block stack.

The GPT model forward pass applies dropout to input embeddings, chains 12 transformer blocks using `nn.Sequential`, applies layer normalization, and outputs logits [c992]. Using `nn.Sequential` is convenient because it allows us to pass the tensor through all 12 blocks with a single call.
...
To keep gradients flowing through 12 stacked blocks, each transformer block uses shortcut connections. A shortcut connection (residual connection) adds the output of a layer back to its input to provide an alternative gradient flow path and prevent the vanishing gradient problem [c950].

#### Layer Normalization

After the transformer block stack, a final layer normalization is applied before the output head. Layer normalization is a technique that normalizes token embeddings by setting the mean to zero and variance to one for each token independently [c943]. More precisely, layer normalization normalizes embedding values for each token such that the mean of the resultant embedding values is zero and the variance is equal to one [c963].

#### The Output Head

The final component is a linear projection that maps from embedding space to vocabulary space. The output head is a neural network at the final stage of the GPT model that transforms token embeddings into logits for vocabulary prediction [c965]. The logits matrix is the output produced by passing token embeddings through the output head neural network [c968].

The output head is the only stage in the GPT model where the input dimensions change from `(batch_size, num_tokens, 768)` to `(batch_size, num_tokens, 50257)` [c973]. This is a critical observation: every other layer in the model preserves the `768`-dimensional embedding size; only the final linear layer expands to the full vocabulary size.
...
The GPT model input is a tensor of shape `(batch_size, sequence_length)` containing token IDs [c990]. Suppose we have a batch of 2 sequences, each of length 4. The input tensor has shape `(2, 4)`.
...
Once the model is instantiated, we can count its parameters. The standard idiom is:
...
Both the token embedding layer and the output layer have shape `(50257, 768)` [c1007]. This is not a coincidence — they operate on the same vocabulary and embedding space, just in opposite directions. The token embedding maps from token IDs to vectors; the output head maps from vectors back to token ID scores.
...
When output layer parameters are removed from the total, the parameter count becomes exactly 124 million, matching the original GPT-2 model size [c1008]. This is where the "124M" in "GPT-2 124M" comes from.
...
[FIGURE: Parameter count breakdown — token embedding (50257×768 = 38.6M), positional embedding (1024×768 = 0.8M), 12 transformer blocks (~85M), layer norm, output head (50257×768 = 38.6M); weight tying removes the output head count to reach 124M total]
...
The full GPT architecture has been implemented and a GPT model instance has been initialized with random weights [c895]. At this point the model is fully functional for inference — it just produces random outputs because the weights have not been trained yet. The model can take an input, run inference through the 124 million parameter GPT architecture, and predict outputs on a laptop [c897].
...
We now have a model that produces a `(batch_size, sequence_length, vocab_size)` tensor of logits. The goal is to convert this output tensor into predictions of the next word [c817]. This section implements the full autoregressive generation loop.

#### The Five-Step Decoding Algorithm

The text generation algorithm consists of five sequential steps [c855]:

1. **Examine the output tensor** — inspect the logits produced by the model.
2. **Extract the last vector** — take only the logits corresponding to the final token position, since that is where the next-token prediction lives.
3. **Convert logits to probabilities** — apply softmax.
4. **Identify the index of the largest value** — apply argmax to find the most likely next token.
5. **Append the token ID to previous inputs** — extend the context and repeat until `max_new_tokens` is reached.
...
Here is the complete implementation, built from its constituent pieces:

**Step 1: Crop the context window.**

Because the positional embedding table only has `context_size` entries, we must ensure we never feed the model more tokens than it can handle:
...
**Step 3: Extract the last-position logits.**
...
The `generate_text_sample` function is called with parameters: model, inputs, `max_new_tokens=6`, and `context_size=1024` [c890]. Starting with input `'hello i am'` and `max_new_tokens=6`, the GPT model generates the complete output `'hello i am a model ready to help'` [c854]. Of course, with randomly initialized weights the output will be nonsense — but the machinery is correct.

---

### Greedy Decoding and Its Limitations

The decoding strategy used in `generate_text_simple` is called **greedy decoding**: at each step, we always pick the single most probable token. This is simple and deterministic, but it has a well-known limitation.

In GPT training, sampling techniques will modify the softmax output so the model does not always select the most likely token, introducing variability and creativity in generated text [c878]. Greedy decoding tends to produce repetitive, bland text because it never explores lower-probability but potentially more interesting continuations. Techniques such as temperature scaling, top-k sampling, and nucleus sampling address this — but they are topics for a later chapter. For now, greedy decoding is sufficient to verify that the architecture is working correctly.
...
**1. Configuration.** We define `GPT_CONFIG_124M` with `vocab_size=50257`, `context_length=1024`, `emb_dim=768`, `n_heads=12`, `n_layers=12`, `drop_rate=0.1`, `qkv_bias=False` [c438].

**2. Model instantiation.** `model = GPTModel(GPT_CONFIG_124M)` creates a model with randomly initialized weights [c895, c996].

**3. Input preparation.** A tokenizer converts the input string into a list of token IDs. These are wrapped in a `(1, seq_len)` tensor. The GPT model input is a tensor of shape `(batch_size, sequence_length)` containing token IDs [c990].

**4. Forward pass.** The model maps the input through token embeddings, positional embeddings, dropout, 12 transformer blocks, layer normalization, and the output head [c811]. The output has shape `(batch_size, sequence_length, 50257)` [c994].
...
**6. Decoding.** The resulting token ID sequence is passed back through the tokenizer's decode method to recover a string.
...
We now have a fully assembled, parameter-counted, inference-capable GPT-2 124M model. The model outputs logits which will be converted into tokens and text outputs in a subsequent step [c1021], and the next set of lectures will focus on training the GPT-2 model with 124 million parameters [c906].

Training requires a loss function (cross-entropy over the vocabulary), an optimizer, a data loader that feeds batches of text, and a training loop that updates the weights. Once trained, the same `generate_text_simple` function — or a more sophisticated sampling variant — will produce coherent, meaningful text rather than random token sequences.

The architecture we have built is not a toy. It is the same fundamental design used in GPT-2, and understanding it from the ground up — every embedding lookup, every attention head, every residual connection — gives you the foundation to understand every subsequent advance in the field.