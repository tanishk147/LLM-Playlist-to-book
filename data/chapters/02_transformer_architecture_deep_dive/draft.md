...
Before diving into history, it helps to have a rough map of the destination. The transformer is, at its simplest, a neural network whose parameters are optimized through training to predict output words correctly [c1276]. It converts input text to output text through a structured pipeline [c1268], and in its original form that pipeline was designed specifically for machine translation—converting English text into German and French [c1319, c1269].
