...
At this point in the book, you have built every individual piece of the GPT-2 architecture: tokenizers, embeddings, multi-head attention, feed-forward networks, layer normalization, shortcut connections, and transformer blocks. This chapter is about snapping all those pieces together into a single, runnable model class, verifying that the parameter count matches the published GPT-2 specification, and then writing the autoregressive loop that turns a prompt into generated text.

By the end of this chapter you will have a working GPT-2 implementation that fits on a laptop and can produce output tokens—albeit nonsensical ones until training is applied.

---

### The Big Picture: What the GPT Pipeline Looks Like

Before writing a single line of code, it helps to have the full data-flow in mind.

[FIGURE: Left-to-right pipeline diagram: input sentence → token IDs → token embeddings (dim 768) → add positional embeddings (dim 768) → input embeddings → dropout → 12× transformer block → layer normalization → output head → logits tensor (vocab_size 50257)]
...
Throughout this chapter we will refer to a configuration dictionary that holds all the hyperparameters in one place. The key values for the 124 M parameter GPT-2 model are:
...
A useful intermediate step before writing the full model is to build a *dummy* version where the transformer blocks and layer normalization are replaced with placeholder classes. This lets you verify that the overall data flow—shapes, device placement, and forward-pass logic—is correct before the heavier components are plugged in.

The following class captures the complete structural skeleton of GPT-2:
...
Even in this skeleton form, every major design decision of the real model is visible. Let's walk through each component.
...
The model's first job is to map integer token IDs to dense vectors. Token embeddings are of dimension 768 [c818]. The embedding table has shape `(vocab_size, emb_dim)` = `(50257, 768)` [c1007].

#### Positional Embeddings

Raw token embeddings carry no information about *where* in the sequence a token appears. Positional embedding is a vector representation of the position of a token within a sequence [c926]. Like the token embedding table, the positional embedding table has shape `(context_length, emb_dim)`.
...
To put it concretely: if you feed in a batch of 2 sequences each of length 4, you get back a tensor of shape `(2, 4, 50257)`. Each of the 4 positions has a 50257-dimensional vector of unnormalized scores, one per vocabulary token. The model outputs logits that can be decoded to predict the next token [c916].
...
One of the first sanity checks after building a model is verifying its parameter count. You can count the total number of parameters in a model's parameter tensors using `p.numel()` [c1004]:
...
Running this on the assembled model gives approximately 163 million parameters. But the published GPT-2 small model has 124 million. Where does the discrepancy come from?

The answer is **weight tying**. Weight tying is a technique where the original GPT-2 architecture reuses the weights from the token embedding layer in its output layer [c1006]. The token embedding layer and output layer in the implemented model both have shape `(50257, 768)` [c1007], so they each hold 50257 × 768 ≈ 38.6 million parameters. When output layer parameters are removed from the total, the parameter count becomes exactly 124 million, matching the original GPT-2 model size [c1008].

Weight tying reduces memory footprint from 163 million to 124 million parameters and reduces computational complexity [c1009]. However, using separate token embedding and output layers results in better training and model performance compared to weight tying [c1010]. For this implementation, we keep the layers separate and simply note that the "124 M" label refers to the weight-tied count.
...
Having a model that outputs logits is only half the story. We need a function that repeatedly queries the model to produce a sequence of tokens. This is the `generate_text_simple` function.
...
2. Extract the last vector (the logits for the *next* token position).
3. Convert logits to probabilities via softmax.
4. Identify the index of the largest probability value using argmax.
5. Append the token ID to the previous inputs, and repeat until `max_new_tokens` is reached.
...
**Step 3 – Extract the last token's logits.**
...
Softmax is applied to convert logits into a probability distribution before finding the index with the highest value using argmax [c874]. Softmax is used in the current code to show the full process of transforming logits to probabilities, providing additional intuition even though it is technically redundant for argmax selection [c877]. Intuitively, argmax of logits and argmax of softmax(logits) always agree because softmax is a monotone transformation—but keeping softmax in the code makes the probability interpretation explicit.
...
**Step 6 – Append the new token and loop.**
...
The next token predictions from the untrained model produce random text because the model parameters have not been trained [c896]. The GPT architecture implementation has not yet undergone training [c905]. This is entirely expected: the weights are initialized randomly, so the probability distributions over the vocabulary are essentially uniform noise.

In GPT training, sampling techniques will modify the softmax output so the model does not always select the most likely token, introducing variability and creativity in generated text [c878]. The greedy argmax strategy used in `generate_text_simple` is the simplest possible decoding strategy—always picking the single most probable token. Real deployments use temperature scaling, top-k sampling, or nucleus sampling to produce more varied and natural-sounding text, but those techniques build on exactly the same logit → probability → token pipeline we have implemented here.
...
This chapter assembled the complete GPT-2 model from its constituent parts. Here is a concise recap of the key points:

| Component | Role | Shape change? |
|---|---|---|
| Token embedding | Map token IDs → dense vectors | `(B, T)` → `(B, T, 768)` |
| Positional embedding | Encode sequence position | Added to token emb, no shape change |
| Dropout | Regularization | No shape change |
| 12× Transformer block | Contextual representation | No shape change |
| Final layer norm | Stabilize activations | No shape change |
| Output head | Project to vocabulary | `(B, T, 768)` → `(B, T, 50257)` |
