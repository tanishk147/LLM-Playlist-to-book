...
LLM decoding strategies are techniques used to control randomness and improve quality when generating the next token in sequence [c750]. The naive strategy selects the generated token corresponding to the largest probability score among all tokens in the vocabulary [c751]. We need something better.

#### Temperature Scaling

Temperature scaling is a technique used in large language models to control randomness and diversity in generated text [c3025]. Concretely, temperature scaling divides the logits by a number greater than zero before applying softmax to convert them into probabilities [c3058, c753].
...
Here is what low-temperature scaling looks like in code:
...
The multinomial function samples the next token proportional to its probability score [c3049]. Multinomial distribution sampling selects the next token probabilistically proportional to its probability score rather than deterministically choosing the maximum probability token [c754].

PyTorch's `torch.multinomial` implements this directly. Here are three illustrative examples:
...
2. Apply top-k sampling: keep only the top-k logits.
...
Text generation strategies including temperature scaling and top-k sampling can be integrated together to reduce overfitting in text generation [c724]. Top-k sampling and temperature scaling are decoding strategies that help avoid overfitting [c798]. Probabilistic decoding strategies prevent the GPT model from memorizing passages when predicting next words [c797].
...
The embedding dimension is the size of the vector space into which each token ID is converted; for GPT-2 small, each token is represented as a 768-dimensional vector [c3492]. N_heads is the number of attention heads present in each transformer block [c3494]. N_layers is the number of transformer blocks in the model [c3495].

#### Downloading the GPT-2 Files

OpenAI originally saved GPT-2 weights using TensorFlow, while the lecture series code uses PyTorch [c3469]. Pre-processing steps are required after downloading GPT-2 weights before they can be integrated with the GPT architecture [c3476].

The download function handles fetching all necessary files from OpenAI's public storage:
...
WTE (token embeddings) stores parameters for converting input token IDs into token embeddings [c3506]. WPE (positional embeddings) stores parameters for adding positional information to token embeddings [c3507]. The blocks key contains all trainable weights and parameters within the transformer blocks, including attention layers, feed-forward networks, output projections, and layer normalization parameters [c3511].

One important structural difference: in GPT-2, the query, key, and value weight matrices in the attention layer are fused into a single large matrix called `C_ATTN` [c3513]. Our custom model stores them separately, so we need to split this matrix when loading. Layer normalization in GPT-2 includes trainable scale (G) and shift (B) parameters applied after mean subtraction and variance normalization [c3517].

[FIGURE: Diagram showing how the fused C_ATTN matrix in GPT-2 maps to separate Q, K, V matrices in the custom model]

#### Loading TensorFlow Checkpoint Variables
