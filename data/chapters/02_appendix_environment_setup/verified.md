...
One of the first decisions your code must make at runtime is which hardware device to use for computation. PyTorch supports three backends that are relevant here: CPU, CUDA (for NVIDIA GPUs), and MPS (Metal Performance Shaders, for Macs with Apple Silicon chips) [c2307].
...
If you are on a MacBook with an Apple Silicon chip, you can go one step further and set the device to `torch.device('mps')` for faster execution [c2310]. Apple Silicon is approximately 2× faster than the Apple CPU for these workloads [c3861], so it is worth the extra line if you have the hardware.

Intuitively, the device variable is passed around to every tensor and model call so that all computation happens in the same memory space. Mixing devices — for example, keeping your model on GPU while your data stays on CPU — is a common source of runtime errors, so it pays to establish a consistent device selection strategy early.

#### Moving Data to the Device During Loading

A subtle but important optimization concerns *when* you transfer tensors to the target device. The recommended approach is to perform the device transfer inside a custom collate function, which runs as part of the data loading pipeline rather than inside the training loop [c2306, c2308]. Because the collate function executes in background worker processes, the transfer happens concurrently with the forward and backward passes, preventing it from blocking the GPU during model training [c2308].

### TensorFlow and tqdm: Loading GPT-2 Weights

Later in the book you will load the original GPT-2 weights released by OpenAI. There is a small compatibility wrinkle: OpenAI originally saved those weights using TensorFlow, while the code in this series uses PyTorch [c3469]. The weights therefore cannot be loaded directly with `torch.load`; TensorFlow must be installed as an intermediary [c3470].
...
tqdm is installed specifically to track the download progress of the weight files [c3471], which can be several gigabytes. Having a progress bar is a small quality-of-life improvement that makes it clear the download has not stalled.
...
Once you have trained or fine-tuned a model, you will want to compare its outputs against a strong reference. Ollama is a lightweight application designed to run large language models on a laptop [c648]. It supports several well-known open-source models, including:
...
An important distinction: Ollama is an inference-only tool [c657]. You perform inference with an already pre-trained model; Ollama does not support training or fine-tuning [c653, c657]. Think of it as a convenient local server for querying models, not a training framework.

#### Installing Ollama

Ollama is available for Mac OS, Linux, and Windows [c658]. The Mac OS installer weighs in at 177 MB [c659], so the download is quick. The model files themselves are a different story — the Llama 3 8-billion-parameter model is 4.7 GB [c664] and takes roughly 15 to 20 minutes to download on a typical connection [c665].
...
One operational detail to keep in mind: Ollama must remain running in the terminal for any downstream Python code to reach it [c668]. Closing the terminal window will stop the server. A good habit is to open a dedicated terminal tab for Ollama and leave it untouched while you work in other tabs.

#### Talking to Ollama from Python

Typing prompts directly in the terminal is fine for quick checks, but for systematic evaluation you will want to call Ollama programmatically. Rather than using the `ollama run` command each time, the recommended approach is to interact with the model through an API from Python [c670]. You can verify that the Ollama session is reachable by checking whether `ollama_running` equals `true` in your verification code [c669].
...
The next chapter begins building the model itself. Having your device selection sorted out now means you can drop `device` into model and data-loader code without revisiting this setup.