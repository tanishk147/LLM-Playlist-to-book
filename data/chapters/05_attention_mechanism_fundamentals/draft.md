...
The attention mechanism is the engine at the heart of every modern large language model. Before we can implement it properly, we need to understand *why* it exists—what problems it solves, and what came before it. This chapter traces the intellectual lineage from recurrent neural networks through Bahdanau attention to the self-attention mechanism used in GPT and the Transformer family. Along the way, we will build a working implementation of simplified self-attention from scratch in Python, step by step.

---

### The Problem: Sequential Data and Long-Range Dependencies

Language is sequential. The meaning of a word depends on the words around it, sometimes on words that appeared many sentences earlier. This is the core challenge that every language model must confront.
...
In sequence-to-sequence tasks like machine translation, an RNN encoder reads the entire source sentence and compresses it into a single vector—the **context vector**—which is the final hidden state of the encoder [c1504]. The decoder then uses this context vector to generate the target sequence one token at a time.
...
RNNs struggle with what are called **long-term dependencies**: complex sentence structures where multiple clauses or phrases are connected, making it difficult for the model to identify relationships between distant words [c1481]. The deeper problem is that gradients tend to shrink as they are propagated back through many time steps, making it hard for the network to learn connections between tokens that are far apart.
...
Intuitively, imagine trying to summarize a 500-word paragraph into a single sentence, then asking someone to reconstruct the original from only that summary. Information is inevitably lost.
...
[FIGURE: Encoder-decoder diagram with Bahdanau attention: encoder produces hidden states h_1 through h_n; at each decoder step, attention weights alpha_1 through alpha_n are computed and used to form a weighted sum of encoder states, producing a step-specific context vector]
...
This is a qualitative shift. Traditional attention looks at one input sequence and one output sequence, determining which parts of the output sequence are more related to which parts of the input sequence [c1558]. The decoder is no longer forced to work from a single compressed summary; it can reach back and attend to whichever encoder states are most relevant at each moment.

---

### From Attention to Self-Attention

The Transformer architecture, introduced in 2017, was built with the self-attention mechanism at its core, which was itself inspired by the Bahdanau attention mechanism [c1534]. But there is a crucial conceptual shift between Bahdanau attention and self-attention.
...
More formally: self-attention is a mechanism that allows each position of an input sequence to attend to all positions in the same sequence [c1548]. The result is that every token in a sequence can directly "see" every other token, regardless of how far apart they are. This is the property that finally solves the long-range dependency problem at scale.
...
Vector embeddings capture semantic meaning, such that semantically related words like "journey" and "starts" are positioned closer to each other in vector space [c2181]. This geometric property is what makes dot-product attention scores meaningful.
...
The concept of a **query** is central here. A query is the element or token being examined; for example, the word "journey" ($x_2$) is the query when we are computing the context vector for that token [c2183]. The attention score between the query and each other token tells us how much the query should attend to that token.
...
where $\theta$ is the angle between the vectors [c2193]. In self-attention mechanisms, the dot product determines the extent to which elements of a sequence attend to one another [c2199]. Intuitively, vectors that point in similar directions (small $\theta$, large $\cos\theta$) will have a large dot product, meaning the query attends strongly to that token.
...
Raw attention scores are useful, but they are not directly interpretable. We want to know, as a fraction, how much attention the query pays to each token. This requires **normalization**.
...
The standard normalization function used in attention is **softmax**. Softmax normalization takes the exponent of every element and divides by the summation of all exponents [c2216]:
...
The `dim` parameter in `torch.softmax` specifies the dimension of the input tensor along which the normalization will be computed [c2265]. When we have a matrix of attention scores, we normalize along the last dimension (the key dimension) so that each row of attention weights sums to one.
...
Once we have attention weights, we compute the **context vector** for the query token. The context vector $z_i$ is defined as:

$$Z_i = \sum_j \left(\text{attention\_weight}_j \times \text{embedding\_vector}_j\right)$$
...
Each row of this matrix is a probability distribution over the input tokens, telling us how the corresponding query distributes its attention across the sequence.

---

### Implementing Simplified Self-Attention in Python

We now have everything we need to implement simplified self-attention from scratch. The implementation follows directly from the three steps above: compute dot-product attention scores, normalize with softmax, and compute the weighted sum.

#### Computing Keys, Queries, and Values (Simplified)

In the simplified version, there are no separate weight matrices. The input embeddings themselves serve directly as queries, keys, and values. We compute attention scores as dot products between input vectors, normalize, and sum.
...
And the context vector is then:
...
The simplified self-attention mechanism we have built is mathematically clean and pedagogically useful, but it has a significant limitation: it can only attend based on the raw semantic similarity of the input embeddings. Using only the dot product to compute attention weights means attention is based solely on semantic similarity, which fails to capture contextual importance when semantically unrelated words are important in context [c2278].

Consider a sentence where a grammatically important function word (like "not") needs to strongly influence the interpretation of a semantically distant content word. The raw embeddings of "not" and that content word may not be particularly similar, so a pure dot-product attention mechanism would assign low attention weight between them—even though their relationship is crucial for understanding the sentence.

This is why trainable weights are needed. Trainable weights in attention allow the model to learn attention patterns beyond semantic similarity, enabling it to assign high attention to contextually important words even when they are not semantically related to the query [c2279]. Trainable weights allow the attention mechanism to capture long-range dependencies by learning contextual patterns beyond the semantic embedding space [c2282].
...
Intuitively, the query asks "what am I looking for?", the keys say "here is what each token offers as a label," and the values say "here is the actual content you get if you attend to me." The attention weights are computed by matching queries against keys, and then those weights are used to aggregate the values.

[FIGURE: Three-panel diagram showing input x being projected through W_Q, W_K, W_V to produce query, key, and value vectors; queries and keys interact via dot product to produce attention scores; attention weights applied to values produce context vectors]
...
The attention scores are then computed between the projected queries and keys, scaled, and normalized:
...
And the context vector is the weighted sum of the projected values:
...
In PyTorch, `torch.nn.Parameter` is a Tensor subclass used to mark tensors as module parameters, which are automatically added to the module's parameter list and appear in the `parameters()` iterator [c1367]. This is the standard way to register learnable parameters inside an `nn.Module`.
...
# Source: [c1368]
```python
# W_q, W_k, W_v are each initialized as:
torch.nn.Parameter(torch.rand(D_in, D_out))
```

Here `D_in` is the dimensionality of the input embeddings and `D_out` is the dimensionality of the projected query/key/value space.

#### Organizing Self-Attention as a PyTorch Class

Self-attention is organized as a Python class (`SelfAttentionVersion1`) derived from `nn.Module` to enable reusable instantiation and integration into larger LLM implementations [c1439]. This design pattern is standard in PyTorch: by subclassing `nn.Module`, the weight matrices are automatically tracked, can be moved to GPU, and will be updated by the optimizer during training.
...
[GAP: An explicit explanation of why scaling by sqrt(d_k) prevents vanishing gradients or softmax saturation is not present in the supplied claims. The scaling factor is defined and used, but the motivation is not stated in the source material.]
...
We have now covered the full arc from RNNs to simplified self-attention, and introduced the three-matrix formulation that underlies the actual mechanism used in modern LLMs. To summarize the conceptual journey:
