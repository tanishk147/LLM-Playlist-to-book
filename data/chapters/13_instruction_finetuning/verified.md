## Instruction Fine-Tuning: Building a Personal Assistant
 ...
 That model can complete sentences and generate coherent prose, but it cannot reliably *follow instructions*. This chapter closes that gap.
...
### Why Instruction Fine-Tuning?
 ...
 Instruction fine-tuning is the process of training a pre-trained LLM with a specific dataset to teach it to correctly follow instructions [c1722]. The mechanism is straightforward: you collect a large set of instruction-response pairs and use them to update the model's weights [c181].
...
Fine-tuning serves two main purposes: training the LLM to follow instructions and training on domain-specific data relevant to the deploying organization [c177]. To make this concrete, consider a healthcare virtual assistant. Fine-tuning it with domain-specific instructions ensures it understands medical terminology, follows healthcare guidelines, and personalizes responses based on patient history [c176]. More generally, building a personal assistant requires the LLM to follow instructions such as correcting grammar, removing filler words, making text concise, and adjusting tone [c168].
 ...
 Fine-tuning on a specific dataset after loading pre-trained weights requires significantly less computational time than training from scratch [c1733]. 
...
- *Response:* The artist composed the song.
 ...
 This distinction matters when we format the prompt.
...
The fine-tuning process begins by loading a training dataset consisting of instruction-response pairs [c182]. The test set is used to evaluate the model's performance after fine-tuning [c605].
...
The following function converts a dataset entry into an Alpaca-formatted prompt string. 
...
### Data Batching: From Text to Tensors
 ...
 The five-step process is [c2294]:
...
#### Step 1: Format with the Prompt Template
 ...
 Each entry in the dataset is passed through this function to produce a fully formatted string [c2291].
...
#### Step 3: Padding to Equal Length
 ...
 In GPT-2, the end-of-text token has token ID `50256` [c1863]. 
...
#### Step 5: Replacing Padding Tokens with `-100`
 ...
 PyTorch's cross-entropy loss function has a built-in `ignore_index` parameter; when a target token ID equals `-100`, that position is excluded from the loss calculation entirely [c1902].
...
Additionally, to mask target token IDs, we replace the token IDs of the instruction and input portions in the target text with `-100` [c1917]. 
...
```
 ...
 The `DataLoader` wraps the dataset and calls the custom collate function to assemble batches.

Stage one of instruction fine-tuning focuses entirely on preparing the dataset itself [c2335]. 

---

### Loading the Pre-Trained Model

With the data pipeline in place, we turn to the model. A pre-trained GPT-2 model with 355 million weights was loaded as the foundation for instruction fine-tuning [c591].
...
Cross-entropy loss is used to calculate the loss on a batch of data [c3118]. 
...
For scalable evaluation, we use a larger language model as a judge. Method 3 LLM evaluation involves comparing the true expected output against the model response by asking a large language model to assign a score based on this comparison [c627]. Step 7 of evaluating a fine-tuned LLM involves implementing automated response evaluation using a larger pre-trained LLM [c646].
 ...
 Aggregating these scores across the test set gives a quantitative measure of fine-tuning quality.

---

### Running Llama 3 Locally with Ollama

 Ollama is an efficient application to run large language models on your laptop [c648]. It handles model downloading, quantization, and inference in a single lightweight binary.

Importantly, Ollama is a tool for generating text using LLM inference and does not support training or fine-tuning LLMs [c657]. 
