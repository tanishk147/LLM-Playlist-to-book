...
Fine-tuning is the process of adapting a pre-trained model to a specific task by training the model on additional data [c2337, c3575]. Rather than training a model from scratch—which requires enormous compute and data—we start from a foundation model that has already learned rich representations of language, then nudge its weights toward a narrower objective [c169]. 
...
The original dataset is imbalanced: there are 4,825 ham (non-spam) messages and 747 spam messages [c2828]. We will address this by balancing the dataset before training.
...
Because the original dataset has approximately 4,800 ham messages and 747 spam messages [c50], we need to downsample the majority class. Intuitively, a balanced dataset prevents the model from learning a trivial shortcut—predicting the majority class every time.
...
#### 10.3.1 Dataset vs. DataLoader
 ...
 A `Dataset` stores the samples and their corresponding labels [c2350]. A `DataLoader` wraps an iterable around the `Dataset` to enable easy access to the samples [c2351]. A PyTorch dataset specifies how data is loaded and processed before instantiating a data loader [c2374].
...
An end-of-text token is a special token used to mark the end of a document or sentence in tokenization [c2363]. 
...
The large language model architecture takes input text and outputs a classification of whether the email is spam or not spam [c2348]. 

#### 10.3.4 Creating the DataLoaders

 The `DataLoader` handles shuffling (for the training set), batching, and iteration. 
...
```
 ...
 The following helper loads them into a nested Python dictionary:
...
The pre-trained GPT-2 model without fine-tuning struggles to follow instructions and lacks classification capabilities for tasks like spam detection [c2851]. Without fine-tuning, the pre-trained GPT model could not correctly answer spam classification prompts even when given explicit instructions [c159]. We need to modify its architecture.
...
A classification head is a small linear layer added on top of a pre-trained language model (like GPT) to adapt it for classification tasks by mapping hidden states to class logits [c2857].
 ...
 For binary classification we only need two logits. To adapt GPT-2 for text classification, we replace the original 768→50257 output layer with a 768→2 linear layer that outputs logits for two classes (spam vs. no spam) [c2854].
...
#### 10.4.4 Which Tokens to Use
 ...
 For spam classification, we focus fine-tuning on the last output token rather than all output rows, since the last token contains all necessary information [c2879]. 
...
Three components will be fine-tuned: the classification head, the final (12th) transformer block, and the final layer normalization module [c2863]. 
...
Predicted labels are the class predictions generated by applying argmax to model logits [c70]. Output labels are also called target labels, representing the true values in the dataset [c69]. 
...
`torch.nn.functional.cross_entropy()` is the PyTorch function used to compute categorical cross-entropy loss [c93]. 
...
The high-level structure is:
 ...
 At regular intervals (controlled by evaluation frequency), compute and print training and validation loss [c110].
...
Step 7 of the fine-tuning training process: after every epoch, calculate and print training accuracy and validation accuracy [c114]. 
...
Fine-tuning is the process of changing model parameters and retraining on a specific dataset so the model performs well on that particular task [c160]. 

### 10.7 Evaluating the Fine-Tuned Model

#### 10.7.1 Train, Validation, and Test Accuracy

 The training and validation accuracies were monitored during training; the test accuracy is the number we care about most, because it reflects performance on data the model has never seen [c137].
...
#### 10.7.2 Comparing to the Unmodified Model
 ...
 The pre-trained GPT-2 model without fine-tuning struggles to follow instructions and lacks classification capabilities for tasks like spam detection [c2851]. Without fine-tuning, the pre-trained GPT model could not correctly answer spam classification prompts even when given explicit instructions [c159]. 
...
The inference procedure mirrors the training forward pass:
 ...
 Extract the last token's output: `outputs[:, -1, :]` [c2880].
5. 
