...
Before a large language model can process text, that text must be converted into a form the model can actually work with: numbers. The bridge between raw text and numerical representations is **tokenization**. This chapter walks through the entire tokenization pipeline, from the simplest possible approach (splitting on whitespace) to the production-grade algorithm used by GPT-2. Along the way, you will build a tokenizer class from scratch, understand why special tokens exist, and learn to use the `tiktoken` library for BPE-compatible tokenization.
...
There are three main types of tokenization algorithms: word-based tokenizers, sub-word-based tokenizers, and character-based tokenizers [c2419]. Each makes a different trade-off between vocabulary size, handling of rare words, and semantic expressiveness. Understanding all three is essential before committing to any one approach.
...
The downside is that sequences become very long, and individual characters carry little semantic meaning. The model must learn to compose meaning from scratch at every level.
...
The key insight is that common words remain whole tokens, while rare words are broken into recognizable pieces. This retains semantic similarity by sharing root word tokens across related words like "tokens" and "tokenizing" [c2952]. Byte pair encoding (BPE) is the most widely used algorithm for implementing this approach [c2953], and it is the algorithm used by GPT models [c2948].
...
A naive whitespace split misses punctuation. A better approach uses a regular expression that splits on both whitespace and common punctuation marks:
...
Sorting ensures that the same text always produces the same vocabulary, which matters for reproducibility.
...
The `SimpleTokenizer` above will raise a `KeyError` if it encounters a word not in its vocabulary. This is the OOV problem in action. The solution is to add **special tokens** to the vocabulary.
...
The end-of-text token (EOS) is a special token inserted between unrelated text sources to signal boundaries and prevent the model from mixing separate documents [c3236]. This is important when training on a corpus assembled from many independent texts: without a boundary marker, the model might learn spurious relationships between the end of one document and the beginning of the next.
...
With this change, any word outside the vocabulary is silently replaced by the `<|unk|>` token ID rather than crashing.
...
Even with special tokens, word-based tokenization has a fundamental ceiling. Consider the words "old", "older", "finest", and "lowest". Word-based tokenization treats each unique word as a single token, resulting in four tokens for this dataset [c2466]. The model has no way to know that "old" and "older" share a root, or that "-est" is a superlative suffix. Every morphological variant is an opaque integer.

Subword tokenization solves this by retaining root words and breaking down rare words into subword units [c2959]. The model can then learn that "old" and "older" share the token "old", and that "-er" is a productive suffix.
...
The core idea is elegant: start with a character-level vocabulary and iteratively merge the most frequent adjacent pair of symbols into a new symbol.
...
In byte pair encoding, the algorithm identifies the most frequently occurring byte pair and replaces it with a new variable that does not occur in the data [c2956]. This merge is recorded and applied consistently across the entire corpus.

Intuitively, if "l o" appears very frequently, merging it into "lo" reduces the total number of tokens in the corpus. The algorithm repeats this process for a fixed number of merge operations, or until a target vocabulary size is reached.
...
Suppose our corpus contains only four word types with the following frequencies:
...
After adding end-of-word markers and splitting into characters, the initial token inventory is: `o`, `l`, `d`, `</w>`, `e`, `r`, `f`, `i`, `n`, `s`, `t`, `w`. The algorithm counts all adjacent pairs across all occurrences, finds the most frequent, and merges it. This continues until the desired vocabulary size is reached. BPE applied to vocabulary achieves subword tokenization by retaining root words and breaking down rare words into subword units [c2959].
...
The tiktoken BPE tokenizer has an encode method that converts words into token IDs [c2491]. The interface is the same as our `SimpleTokenizer`: call `encode` to go from text to IDs, and `decode` to go back.
...
[GAP: Exact output of tiktoken.encode for a sample string, as a concrete claim from the source material]

#### How the BPE Tokenizer Handles Unknown Words

Unlike our `SimpleTokenizer`, the BPE tokenizer never needs an `<|unk|>` token. The BPE tokenizer breaks down words not in its predefined vocabulary into smaller sub-word units or individual characters [c2504]. Because the base vocabulary includes all individual bytes (256 possible values), every possible string can be encoded without any unknown tokens.
...
1. **Load raw text** from a file or dataset.
2. **Split into tokens** using either regex-based splitting (for a simple tokenizer) or BPE (for production use).
3. **Build or load a vocabulary** that maps each unique token to a unique integer.
4. **Encode** text into token ID sequences using the vocabulary.
5. **Decode** token ID sequences back into text when needed (e.g., to inspect model output).
...
| Property | SimpleTokenizer | tiktoken (GPT-2 BPE) |
|---|---|---|
| Vocabulary source | Built from corpus | Pre-trained on large corpus |
| OOV handling | `<|unk|>` token | Decompose to subwords/chars |
| Vocabulary size | Corpus-dependent | ~50,257 tokens |
| Handles new words | No | Yes |
| Special tokens | `<|unk|>`, `<|endoftext|>` | `<|endoftext|>` |
...
In the next chapter, we will take the token ID sequences produced here and convert them into dense vector embeddings—the actual numerical representations that flow through the transformer's attention layers.