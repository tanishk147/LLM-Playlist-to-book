## Building the Transformer Block: LayerNorm, GELU, Feed-Forward, and Shortcut Connections
 ...
 Before we can assemble it, we need to understand and implement each of its sub-components individually. 
...
#### The Problem: Internal Covariate Shift
 ...
 At each step, the weights of every layer change. This phenomenon is called **internal covariate shift** [c1038].

Formally, internal covariate shift is the problem where the input distribution to each layer changes across training iterations, making weight updates difficult and delaying convergence [c1038]. 
...
#### A Quick Empirical Check
 ...
 The following code creates a two-sample batch, passes it through a linear layer and ReLU, and then we can inspect the output statistics:
...
#### Scale and Shift: Trainable Parameters
 ...
 To give the model the ability to undo normalization when that is useful, layer normalization introduces two trainable parameters: **scale** and **shift** [c1078].
...
The ReLU activation function is simple and effective: it returns the input value for $x > 0$ and returns zero for $x < 0$ [c2002]. 

The dead neuron problem occurs when a neuron's output becomes negative, ReLU sets it to zero, and the neuron stops contributing to the learning process [c2005]. In a large model with millions of neurons, a significant fraction can die during training, wasting capacity.
...
where $\Phi(x)$ is the cumulative distribution function of the standard Gaussian distribution [c2006]. 
...
This matches the approximation formula exactly [c2108]. 
...
The goal of implementing a feed-forward neural network sub-module is to create a component that is part of the LLM transformer block [c1999]. Within the transformer block, the feed-forward network processes each element of the sequence separately, without considering relationships with other elements [c2092]. 
...
`nn.Sequential` is a PyTorch module that chains neural network layers together by forwarding outputs sequentially to inputs of subsequent modules [c2033]. 
...
#### Implementing Shortcut Connections
 ...
 After computing a layer's output, you add the original input back:
...
```
 ...
 To verify that shortcut connections actually stabilize gradients, we can set up a small experiment. First, define a loss function as the squared difference between the model output and the ground truth target [c260]:
...
### Assembling the Transformer Block
 ...
 The transformer block is the fundamental building block of GPT and other LLM architectures [c2051].
...
The original transformer model applied layer normalization *after* the self-attention and feed-forward neural network, a pattern called **post-layer norm** [c2130]. This is the arrangement we implement here, where `norm_one` precedes attention and `norm_two` precedes the feed-forward network [c2121].

#### The Stacking Order

The transformer block stacking order is [c2083]:

 Shortcut connection (add input to output)
...
Previous lectures covered layer normalization, GELU activation, and feed-forward neural networks as building blocks for transformers [c277], and shortcut connections are covered in detail as a building block for understanding the transformer block [c278]. 
...
This chapter built four essential sub-components of the transformer block from scratch:
 ...
 It normalizes along the feature dimension, unlike batch normalization which normalizes along the batch dimension.

 Its approximation formula—$0.5 \cdot x \cdot (1 + \tanh(\sqrt{2/\pi} \cdot (x + 0.044715 \cdot x^3)))$—is the version used in GPT-2 [c2108].

 This allows the model to explore a richer feature space while preserving dimensionality for the residual connection [c2029, c2110].

 The mathematical consequence—a guaranteed +1 term in the gradient—ensures that gradient magnitude remains stable across many layers [c237, c2082].
